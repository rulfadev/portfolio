...
<div>
    ...
    <input type="{{ $type }}" id="{{ $id }}" name="{{ $name }}" value="{{ old($name, $value) }}"
        ...
        {{ $attributes->merge([
            'class' =>
                'form-field-height w-full rounded-2xl border border-white/10 bg-slate-950/85 px-4 text-sm text-white shadow-inner shadow-black/20 outline-none transition placeholder:text-slate-600 hover:border-blue-400/40 focus:border-blue-400 focus:ring-4 focus:ring-blue-600/15 disabled:cursor-not-allowed disabled:opacity-60',
        ]) }}>
